 ...
 * @LastEditTime: 2022-10-28 09:49:53
...
namespace armor_processor
{
    ...
    : Node("armor_processor", options)
    {
        ...
        gimbal_info_pub = this->create_publisher<global_interface::msg::Gimbal>("/gimbal_info", 10);
        ...
        if(processor_->armor_predictor_.debug_param_.using_imu)
        {
            ...
            target_info_sub_.subscribe(this, "/armor_info");
            // point_sub_.subscribe(this, "/armor_detector/armor_point_stamped");
        ...
        else
        {
            ...
            target_info_sub_ = this->create_subscription<global_interface::msg::Target>("/armor_info", 10,
    ...
    std::unique_ptr<Processor> ArmorProcessorNode::init_armor_processor()
    {
        ...
        this->declare_parameter("bullet_speed", 28);
        this->declare_parameter("max_time_delta", 1000);
        this->declare_parameter("max_cost", 509);
        this->declare_parameter("max_v", 8);
        this->declare_parameter("min_fitting_lens", 10);
        this->declare_parameter("shoot_delay", 100);
        ...
        this->declare_parameter("coord_file_path", "/home/zhengtu/Desktop/TUP-Vision-2023-Based/src/global_user/config/camera.yaml");
        coord_file_path = this->get_parameter("coord_file_path").as_string();

        return std::make_unique<Processor>(predict_param, debug_param, coord_file_path);
    }
    ...
    {
        //transform armor_3d position in camera to world
        geometry_msgs::msg::PointStamped point_in;
        point_in.header = target_info.header;
        point_in.point = target_info.aiming_point;
        ...
        try
        {
            ...
            tf2_buffer_->transform(point_in, point_out, this->target_frame_);
            RCLCPP_INFO(
                this->get_logger(), "Point of armor center in frame of world: x:%f y:%f z:%f\n",
                point_out.point.x,
                point_out.point.y,
                point_out.point.z);
        }
        ...
        if(target_info.target_switched)
        {
            ...
            aiming_point[0] = target_info.aiming_point.x;
            aiming_point[1] = target_info.aiming_point.y;
            aiming_point[2] = target_info.aiming_point.z;
            ...
            gimbal_info_pub->publish(gimbal_info);
        }
        else
        {
            Eigen::Vector3d aiming_point;
            ...
            auto angle = processor_->coordsolver_.getAngle(aiming_point_cam, processor_->rmat_imu);
