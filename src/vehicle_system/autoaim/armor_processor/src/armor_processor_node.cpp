 ...
 * @LastEditTime: 2023-05-21 23:01:49
...
namespace armor_processor
{
    ...
    void ArmorProcessorNode::targetMsgCallback(const AutoaimMsg& target_info)
    {
                ...
                if (!target.is_target_lost)
                {
                    ...
                    {   //小陀螺下自动开火判据
                        for (auto armor_point3d_world : armor3d_vec)
                        {
                            ...
                            if (armor3d_dist < min_dist && rangle >= 1.47 && rangle <= 1.52)
                            {
        ...
        if (!target_info.is_target_lost)
        {
            ...
            if (!target.is_spinning)
            {
                Eigen::Vector4d tracking_point3d = {target_info.armors[0].point3d_world.x, target_info.armors[0].point3d_world.y, target_info.armors[0].point3d_world.z, 0.0};
