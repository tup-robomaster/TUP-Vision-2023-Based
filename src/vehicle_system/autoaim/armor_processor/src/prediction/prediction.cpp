 ...
 * @LastEditTime: 2023-04-15 19:56:45
...
namespace armor_processor
{
    ...
    Eigen::Vector3d ArmorPredictor::predict(AutoaimMsg& target_msg, uint64 timestamp, double& delay_time, cv::Mat* src)
    {
        ...
        else if (!filter_disabled_)
        {
            if (target.is_target_switched)
            {
                history_vel_[is_spinning][0][3] = history_vel_[is_spinning][0][2] = history_vel_[is_spinning][0][1] = history_vel_[is_spinning][0][0] = 0.0;
                history_acc_[is_spinning][0][3] = history_acc_[is_spinning][0][2] = history_acc_[is_spinning][0][1] = history_acc_[is_spinning][0][0] = 0.0;
                predict_vel_[is_spinning][0][3] = predict_vel_[is_spinning][0][2] = predict_vel_[is_spinning][0][1] = predict_vel_[is_spinning][0][0] = 0.0;
                predict_acc_[is_spinning][0][3] = predict_acc_[is_spinning][0][2] = predict_acc_[is_spinning][0][1] = predict_acc_[is_spinning][0][0] = 0.0;
                history_vel_[is_spinning][1][3] = history_vel_[is_spinning][1][2] = history_vel_[is_spinning][1][1] = history_vel_[is_spinning][1][0] = 0.0;
                history_acc_[is_spinning][1][3] = history_acc_[is_spinning][1][2] = history_acc_[is_spinning][1][1] = history_acc_[is_spinning][1][0] = 0.0;
                predict_vel_[is_spinning][1][3] = predict_vel_[is_spinning][1][2] = predict_vel_[is_spinning][1][1] = predict_vel_[is_spinning][1][0] = 0.0;
                predict_acc_[is_spinning][1][3] = predict_acc_[is_spinning][1][2] = predict_acc_[is_spinning][1][1] = predict_acc_[is_spinning][1][0] = 0.0;
                error_cnt_ = 0;
                history_info_.clear();
                history_pred_.clear();
            }
        ...
        if ((int)history_info_.size() < predict_param_.min_fitting_lens)
        {
            if(!fitting_disabled_ && is_predicted_ && (int)history_info_.size() > 0)
            {
                ...
                if (to_now_timestamp > target_period_)
                {
                    ...
                    to_now_timestamp -= (to_now_scale) * target_period_;   
                } 
                
                if (to_pred_timestamp > target_period_)
                {
                    int to_pred_scale = to_pred_timestamp / target_period_;
                    to_pred_timestamp -= (to_pred_scale) * target_period_;
                }

                double last_pred_x = fitting_params_[0] * to_now_timestamp + fitting_params_[1];
                double pre_x = fitting_params_[0] * to_pred_timestamp + fitting_params_[1];
                double pred_dx = pre_x - last_pred_x;
                // double delta_y = last_pred_x - target.xyz[1]; 
                // delta_y = abs(delta_y) > 0.1 ? 0.0 : delta_y;
                ...
                if (!target.is_target_switched && last_pred_dx  != 0.0 &&
                (int)history_y_info_.size() != (predict_param_.min_fitting_lens - 1) && last_pred_dx / pred_dx < 0.0
                )
                {
                    pred_dx = last_pred_dx;
                }
                else
                {
                    last_pred_dx = 0.0;
        ...
        if (!filter_disabled_ && !target.is_spinning && (int)history_info_.size() >= predict_param_.min_fitting_lens)
        {
            ...
            {
                updateVel(is_spinning, target_vel);
                updateAcc(is_spinning, target_acc);
            }
        }
            ...
            {   // 基于CS模型的卡尔曼滤波
                ...
                if (debug_param_.x_axis_filter)
                {
                    ...
                    xyz_future[0] = std::async(std::launch::async, [&](){
                        is_singer_available.xyz_status[0] = predictBasedSinger(is_spinning, 0, target.xyz[0], result_singer[0], target_vel[0], target_acc[0], delta_time_estimate);});
                }
                if (debug_param_.y_axis_filter)
                {
                    ...
                    xyz_future[1] = std::async(std::launch::async, [&](){
                        is_singer_available.xyz_status[1] = predictBasedSinger(is_spinning, 1, target.xyz[1], result_singer[1], target_vel[1], target_acc[1], delta_time_estimate);});
                }
                if (debug_param_.z_axis_filter)
                {
                    ...
                    xyz_future[2] = std::async(std::launch::async, [&](){
                        is_singer_available.xyz_status[2] = predictBasedSinger(is_spinning, 2, target.xyz[2], result_singer[2], target_vel[2], target_acc[2], delta_time_estimate);});
                }
        ...
        {   // 曲线拟合预测
            ...
            if (target.is_spinning)
            {
                ...
                if (debug_param_.x_axis_filter)
                {
                    // is_singer_available.xyz_status[0] = predictBasedSinger(0, target.xyz[0], result_singer[0], target_vel[0], target_acc[0], delta_time_estimate);
                    xyz_future[0] = std::async(std::launch::async, [&](){
                        is_fitting_available.xyz_status[0] = predictBasedSinger(is_spinning, 0, target.xyz[0], result_fitting[0], target_vel[0], target_acc[0], delta_time_estimate);}
                    );
                }
                if (debug_param_.y_axis_filter)
                {
                    xyz_future[1] = std::async(std::launch::async, [&](){
                            // is_fitting_available = coupleFittingPredict(true, target, result_fitting, time_estimate);
                            is_fitting_available.xyz_status[1] = predictBasedSinger(is_spinning, 1, target.xyz[1], result_fitting[1], target_vel[1], target_acc[1], delta_time_estimate);
                        }
                    );
                }
                if (debug_param_.z_axis_filter)
                {
                    xyz_future[2] = std::async(std::launch::async, [&](){
                        is_fitting_available.xyz_status[2] = predictBasedSinger(is_spinning, 2, target.xyz[2], result_fitting[2], target_vel[2], target_acc[2], delta_time_estimate);}
                    );
                }

                if (debug_param_.x_axis_filter && xyz_future[0].wait_for(5ms) == std::future_status::timeout)
                {
                ...
                if (debug_param_.z_axis_filter && xyz_future[2].wait_for(5ms) == std::future_status::timeout)
                {
        ...
        {   // Draw curve.
            ...
            {
                curveDrawer(0, *src, predict_vel_[is_spinning][0], cv::Point2i(260, 200));
                // curveDrawer(0, *src, history_acc_[0], cv::Point2i(620, 200));
            } 
            if (is_singer_available.xyz_status[1])
            {
                curveDrawer(1, *src, predict_vel_[is_spinning][1], cv::Point2i(260, 200));
                // curveDrawer(1, *src, history_acc_[1], cv::Point2i(620, 200));
            }
            if (is_singer_available.xyz_status[2])
            {
                curveDrawer(3, *src, predict_vel_[is_spinning][2], cv::Point2i(260, 200));
    ...
    bool ArmorPredictor::predictBasedSinger(bool is_spinning, int axis, double meas, double& result, double target_vel, double target_acc, int64_t timestamp)
    {
        bool is_available;
