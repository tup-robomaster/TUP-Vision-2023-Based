 ...
 * @LastEditTime: 2023-04-10 20:38:34
    ...
    Eigen::Vector3d ArmorPredictor::predict(AutoaimMsg& target_msg, uint64 timestamp, double& delay_time, cv::Mat* src)
    {
        ...
        if ((int)history_info_.size() < predict_param_.min_fitting_lens)
        {
            if(!fitting_disabled_ && is_predicted_ && (int)history_info_.size() > 0)
            {
                ...
                // cout << "xxx to_now_timestamp:" << to_now_timestamp << " to_pred_timestamp:" << to_pred_timestamp << endl;
                ...
                // cout << "to_now_timestamp:" << to_now_timestamp << " to_pred_timestamp:" << to_pred_timestamp << endl;
                ...
                if (!target.is_target_switched)
                {
                    if ((last_pred_dx  != 0.0 && last_pred_dx / pred_dx < 0.0)
                    || (last_pred_dx  != 0.0 && pred_dx / last_pred_dx) > 1.2)
                    {
                ...
                last_last_pred_dx = last_pred_dx;
        ...
        {   // 曲线拟合预测
            ...
            if (target.is_spinning)
            {
                ...
                if (debug_param_.x_axis_filter)
                {
                    // is_singer_available.xyz_status[0] = predictBasedSinger(0, target.xyz[0], result_singer[0], target_vel[0], target_acc[0], delta_time_estimate);
                    xyz_future[0] = std::async(std::launch::async, [&](){
                        is_fitting_available.xyz_status[0] = predictBasedSinger(0, target.xyz[0], result_fitting[0], target_vel[0], target_acc[0], delta_time_estimate);}
                    );
                }
                if (debug_param_.y_axis_filter)
                {
                ...
                {
                    xyz_future[2] = std::async(std::launch::async, [&](){
                        is_fitting_available.xyz_status[2] = predictBasedSinger(2, target.xyz[2], result_fitting[2], target_vel[2], target_acc[2], delta_time_estimate);}
                    );
                }
            ...
            //     is_fitting_available.xyz_status[0] = predictBasedSinger(0, target.xyz[0], result_singer[0], target_vel[0], target_acc[0], delta_time_estimate);
            // }
            // cout << " target:" << target.xyz[1] << " filter:" << result_fitting[1] << endl;
        ...
        result[1] = (abs(target.xyz[1] - result[1]) > 0.30) ? target.xyz[1] : result[1];  
        RCLCPP_INFO_THROTTLE(logger_, steady_clock_, 500, "filter_y:%.4f target_y:%.4f", result[1], target.xyz[1]);
