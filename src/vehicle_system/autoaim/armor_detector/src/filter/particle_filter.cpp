 ...
 * @LastEditTime: 2022-11-29 10:29:59
 * @FilePath: /TUP-Vision-2023-Based/src/vehicle_system/autoaim/armor_detector/src/filter/particle_filter.cpp
 */
...
namespace armor_detector
{
    ...
    bool ParticleFilter::initParam(YAML::Node &config,const string param_name)
    {
        ...
        global_user::initMatrix(process_noise_cov_tmp,read_vector);
        process_noise_cov = process_noise_cov_tmp;
        //初始化观测噪声矩阵
        read_vector = config[param_name]["observe_noise"].as<vector<float>>();
        global_user::initMatrix(observe_noise_cov_tmp,read_vector);
