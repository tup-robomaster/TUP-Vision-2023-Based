 ...
 * @LastEditTime: 2023-05-15 22:49:46
...
namespace armor_detector
{
    ...
    bool Detector::gyro_detector(TaskData &src, global_interface::msg::Autoaim& autoaim_msg, ObjHPMsg hp, DecisionMsg decision_msg)
    {
        ...
        if (trackers_map_.count(target_key) == 0)
        {
            ...
            is_last_target_exists_ = false;
            autoaim_msg.is_target_lost = true;
            RCLCPP_WARN_THROTTLE(logger_, steady_clock_, 50, "No available tracker exists! target_key: %s", target_key.c_str());
            return false;
        }
        ...
        RCLCPP_INFO_THROTTLE(
            ...
            50,
            "spin_state: %s",
            spin_status == UNKNOWN ? "UNKNOWN" : "SPINNING"
        );
        ...
        if (spin_status != UNKNOWN)
        {
            ...
            int armor_size = final_armors.size();
            if (armor_size == 0)
            {
                autoaim_msg.is_target_lost = true;
                lost_cnt_++;
            ...
            if (armor_size == 1)
            {
                ...
                if(spin_status == CLOCKWISE)
                    autoaim_msg.clockwise = CLOCKWISE;
                else
                    autoaim_msg.clockwise = false;
            ...
            else
            {
                ...
                RCLCPP_ERROR(logger_, "Error in dead zone: armor size:%d", armor_size);
                return false;
            }
