 ...
 * @LastEditTime: 2023-05-05 03:31:48
...
namespace armor_detector
{
     ...
     */
    void SpinningDetector::createArmorTracker(std::multimap<std::string, ArmorTracker>& trackers_map, std::vector<Armor>& armors, std::map<std::string, int>& new_armors_cnt_map, int64_t timestamp, bool is_last_exists)
    {
        ...
        */
        ...
        if (isSpinning(trackers_map, now))
        {
            for (auto & tracker : spinning_map_.spin_status_map)
            {
                ...
                //     steady_clock_, 
                //     500, 
                //     "normal_status_counter:%d switch_status_counter:%d", 
                //     spinning_map_.spin_counter_map[tracker.first].normal_gyro_status_counter, 
                //     spinning_map_.spin_counter_map[tracker.first].switch_gyro_status_counter
                // );
            }
        }
