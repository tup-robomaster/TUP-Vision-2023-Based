 ...
 * @LastEditTime: 2023-05-17 15:28:02
            ...
            if (cnt.second == 1)
            {
                ...
                {   // 若相同key键的tracker存在两个，一个为新增，一个先前存在，且两个tracker本次都有更新，则视为一次陀螺动作（其实就是对应目标车辆小陀螺时两个装甲板同时出现在视野的情况）
                    ...
                    if (new_tracker != nullptr && last_tracker != nullptr)
                    {
                        ...
                        // RCLCPP_INFO(
                        //     logger_, 
                        //     "[Spinning]: spin_x_dix:%.3f last_dt:%.5f new_dt:%.5f now:%.5f", 
                        //     abs(spin_x_dis), last_armor_timestamp / 1e9, new_armor_timestamp / 1e9, now / 1e9
                        // );
