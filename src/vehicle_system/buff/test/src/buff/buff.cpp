...
namespace buff
{
    ...
    */
     ...
     * @return false 识别失败
     */
    bool Buff::run(TaskData &src, TargetInfo& target_info)
    {
        ...
        {
            if(debug_param_.detect_buff_red)
                if (object.color != 1)
                    continue;
            if(debug_param_.detect_buff_blue)
                if (object.color != 0)
                    continue;
            ...
            auto pnp_result = coordsolver.pnp(points_pic, rmat_imu_, target_type, SOLVEPNP_ITERATIVE);
