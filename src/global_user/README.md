...
|using_imu         | 是否使用陀螺仪数据，用于装甲板的三维坐标转换到云台坐标系下。             |  若为true，串口驱动节点需能正常工作并发布陀螺仪数据；false则不使用陀螺仪数据。`调试程序前得改！！！`           |
|using_roi          | 是否开启ROI             | 车辆间装甲板不断发生切换可以开启            |
|detect_red          | 是否检测红色            |             |
...
|using_imu            | 是否使用陀螺仪数据   | 若为false，serialport节点不会启动，即无串口调试，`调试时得改！！！`      |
