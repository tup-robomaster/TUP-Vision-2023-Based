 ...
 * @LastEditTime: 2023-03-06 20:46:44
...
namespace serialport
{
    ...
    : Node("serial_port", options), device_name_("ttyACM0"), baud_(115200)
    {
        ...
        {   // Use serial port.
            if (serial_port_->openPort())
            {
                ...
                receive_timer_ = rclcpp::create_timer(this, this->get_clock(), 2ms, std::bind(&SerialPortNode::receiveData, this));
                if (is_sentry_)
                {
    ...
    void SerialPortNode::receiveData()
    {
        ...
        {
            data_transform_->getPosInfo(flag, &serial_port_->serial_data_.rdata[3], vehicle_pos_info_);
        }
        else if (flag == 0xC5)
        {
            vector<ushort> hp;
            ...
            {
                car_pos_msg.pos[ii].x = vehicle_pos_info_[ii];
                car_pos_msg.pos[ii].y = vehicle_pos_info_[ii+1];
            ...
            if (print_referee_info_)
            {
                for(int ii = 0; ii < 20; ii++)
                    RCLCPP_INFO(this->get_logger(), "Pos:%.2f", vehicle_pos_info_[ii]);
