...
namespace serialport
{
    ...
    {
        vector<ushort> vehicle_pos_info;
        while (1)
        {
            // 若串口离线则跳过数据发送
            ...
            if (flag == 0xA5)
            {
                ...
                data_transform_->getThetaAngle(&serial_port_->serial_data_.rdata[55], shoot_delay);
                // Gimbal angle
                ...
                if (print_serial_info_)
                {
                    ...
                    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 500, "shoot_delay:%f", shoot_delay);
                }
                ...
                geometry_msgs::msg::TransformStamped trans_base_to_imu_msg;
                geometry_msgs::msg::Transform trans_base_to_imu_msg_nostamp;
                trans_base_to_imu_msg.transform = tf2::toMsg(trans_imu);
            ...
            else if (flag == 0xC5)
            {
                ...
                data_transform_->getModeSet(flag, &serial_port_->serial_data_.rdata[36], mode_set_pos, mode_set_mode);
    ...
    void SerialPortNode::buffMsgCallback(GimbalMsg::SharedPtr target_info) 
    {
        ...
        {
            if (!sendData(target_info))
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 500, "buff msg received...");
        }
        return;
    }
