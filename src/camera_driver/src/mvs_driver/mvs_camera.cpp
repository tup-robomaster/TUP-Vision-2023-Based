...
namespace camera_driver
{
    ...
    bool MvsCamera::open()
    {
        ...
        if (status = CAMERA_STATUS_SUCCESS)
        {
            RCLCPP_WARN(logger_, "Loaded parameters from cfg file.");

        }
        else
        {
            RCLCPP_WARN(logger_, "Invalid config path: [%d], disabling configure from cfg file now... Error code: [%d]", status);
